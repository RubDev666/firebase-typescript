...
const Producto = () => {
    ...
    const [producto, guardarProducto] = useState<Producto>(inicial);

    const [error, guardarError] = useState(false);
    const [comentario, guardarComentario] = useState<comentarios>({usuarioNombre: '', usuarioId: '', mensaje: ''});
    const [consultarDB, guardarConsultarDB] = useState(true);

    //const usuario = useAutenticacion();
    
    //no uso state, por que no se actualiza bien y es mas complejo...
    let votoUsuarios: string[] = [];
    ...
    const obtenerDatos = async (id: string | string[] | undefined) => {
        let dato;
